...
# Get occurence data, just lat/lon
...
# This downloads 19 raster filed, one for each climate variable
...
# focuses on locations where the data is 
geographicExtent <- extent(x = occurrenceSpatialPts)

# standardize the random points for reproducibility

set.seed(45) 

# Creating random, pseudo-absence points 
...
# change column names from x and y to lon and lat
...
# create a grid of climate measurements , per occurance plot 

occEnv <- na.omit(raster::extract(x = currentClimRasterStack, y = occurrenceCoords))

# create a grid of measurements for pseudo absence points 
